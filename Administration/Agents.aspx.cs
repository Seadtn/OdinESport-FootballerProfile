    ...
    public partial class Agents : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    DeleteProfile(int.Parse(agentId));
                    LoadData();
                }
            }
        ...
        protected void SearchButton_Click(object sender, EventArgs e)
        {
            ...
            string query = "select [Agent].id agentId,[Photo].id ,FirstName, LastName,Path as Profile,DATEDIFF(YEAR, DateOfBirth, GETDATE()) AS Age, Country, DateOfBirth   FROM [Agent],[Photo] where [Photo].id =[Agent].PhotoID ";
            if (fname != "")
            {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlDataAdapter adapter = new SqlDataAdapter(query, connection))
                {
                    ...
                    {
                        tableHtml.Append("<tr>");
                        tableHtml.Append("<td>" + row["agentId"]+ "</td>");
                        ...
                        tableHtml.Append("<a class='btn btn-sm btn-icon btn-success' data-toggle='tooltip' data-placement='top' title='' data-original-title='Add' href='#'>");
                        ...
                        tableHtml.Append("</a>");
                        tableHtml.Append("<a class='btn btn-sm btn-icon btn-warning' data-toggle='tooltip' data-placement='top' title='' data-original-title='Edit' href='ProfilAgent.aspx?id=" + row["agentId"] + "'>");
                        ...
                        tableHtml.Append("</span>");
                        tableHtml.Append("</a>");
                        // Generate the "Delete" column
                        tableHtml.Append("<a class='btn btn-sm btn-icon btn-danger' href='javascript:void(0);' onclick=\"deleteProfile(" + row["agentId"] + ")\" data-id='" + row["agentId"] + "' title='Delete'>");
                        tableHtml.Append("<svg width='24' viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>");
                        tableHtml.Append("<path d='M6 18L18 6M6 6L18 18' stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'></path>");
                        tableHtml.Append("</svg>");
                        tableHtml.Append("</a>");
        ...
        protected void LoadData()
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlDataAdapter adapter = new SqlDataAdapter(query, connection))
                {
                    ...
                    // Fill the DataTable with all data from the database
        ...
        public static void DeleteProfile(int agentId)
        {
            string connectionString = ConfigurationManager.ConnectionStrings["SQLServer"].ConnectionString;
