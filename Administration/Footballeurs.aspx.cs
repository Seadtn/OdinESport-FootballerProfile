...
namespace OdinESport.Administration
{
    public partial class Footballeurs : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            footballerId = Request.QueryString["ProfileId"];
            if (!IsPostBack)
            {
                // Load all data initially when the page loads
                LoadData();

                if (footballerId != "" && footballerId!=null)
                {
                    DeleteProfile(int.Parse(footballerId));
                    LoadData();
                }
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlDataAdapter adapter = new SqlDataAdapter(query, connection))
                {
                    ...
                    foreach (DataRow row in searchResultTable.Rows)
                    {
                        ...
                        tableHtml.Append("</a>");
                        tableHtml.Append("<a class='btn btn-sm btn-icon btn-warning' data-toggle='tooltip' data-placement='top' title='' data-original-title='Edit' href='ProfilFootballeur.aspx?id=" + row["FootballerId"] + "'>");
                        ...
                        tableHtml.Append("<a class='btn btn-sm btn-icon btn-danger' href='javascript:void(0);' onclick=\"deleteProfile(" + row["FootballerId"] + ")\" data-id='" + row["FootballerId"] + "' title='Delete'>");
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlDataAdapter adapter = new SqlDataAdapter(query, connection))
                {
                    ...
                    foreach (DataRow row in dataTable.Rows)
                    {
                        ...
                        tableHtml.Append("<div class='flex align-items-center list-user-action'>");
                        tableHtml.Append("<a class='btn btn-sm btn-icon btn-success' data-toggle='tooltip' data-placement='top' title='' data-original-title='Add' href='#'>");
                        tableHtml.Append("<span class='btn-inner'><svg width='32' viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg' >" +
        ...
        public static void DeleteProfile(int profileId)
        {
            ...
            string query = "DELETE FROM [Footballer] WHERE id = @ProfileId";
