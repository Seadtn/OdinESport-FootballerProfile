...
namespace OdinESport.agents
{
    public partial class Footballeurs : System.Web.UI.Page
    {
        ...
        protected void SearchButton_Click(object sender, EventArgs e)
        {
            ...
            string query = "select [Photo].id ,Path as Profile,DATEDIFF(YEAR, DateOfBirth, GETDATE()) AS Age, Country, DateOfBirth ,Foot, Height, Weight  FROM [Footballer],[Photo] where [Photo].id =[Footballer].PhotoID ";
            if (age != "")
            { 
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlDataAdapter adapter = new SqlDataAdapter(query, connection))
                {
                    ...
                    {
                        tableHtml.Append("<tr>");
                        tableHtml.Append("<td class='text-center'><img class='bg-soft-primary rounded img-fluid avatar-40 me-3' src='../assets/images/avatars/" + row["Profile"] + ".jpg' alt='profile'></td>");
                        ...
                        tableHtml.Append("<td>" + row["Height"] + "</td>");
                        tableHtml.Append("<td>" + row["Weight"] + "</td>");
                        tableHtml.Append("<td>");
                      ...
                      //  tableHtml.Append("</span>");
                        tableHtml.Append("</a>");
                        tableHtml.Append("<a class='btn btn-sm btn-icon btn-warning' data-toggle='tooltip' data-placement='top' title='' data-original-title='Edit' href='ProfilFootballeur.aspx?id=" + row["id"] + "'>");
